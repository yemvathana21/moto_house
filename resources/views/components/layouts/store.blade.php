...
<body class="bg-gray-50 text-gray-900 antialiased font-sans overflow-x-hidden pb-16 md:pb-0">
    ...
    <nav class="md:hidden fixed bottom-0 inset-x-0 bg-white border-t border-gray-200 z-50 flex items-center justify-around px-2 py-1 safe-area-bottom" x-data>
        <a href="/" class="flex flex-col items-center gap-0.5 px-3 py-1.5 rounded-xl {{ request()->is('/') ? 'text-orange-600' : 'text-gray-500' }} transition">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6"/></svg>
            <span class="text-[10px] font-medium">{{ __('Home') }}</span>
        </a>
        <a href="/shop" class="flex flex-col items-center gap-0.5 px-3 py-1.5 rounded-xl {{ request()->is('shop') || request()->is('shop/*') ? 'text-orange-600' : 'text-gray-500' }} transition">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z"/></svg>
            <span class="text-[10px] font-medium">{{ __('Shop') }}</span>
        </a>
        <a href="/cart" class="flex flex-col items-center gap-0.5 px-3 py-1.5 rounded-xl {{ request()->is('cart') ? 'text-orange-600' : 'text-gray-500' }} transition relative">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 100 4 2 2 0 000-4z"/></svg>
            <livewire:cart-counter />
            <span class="text-[10px] font-medium">{{ __('Cart') }}</span>
        </a>
        <a href="/wishlist" class="flex flex-col items-center gap-0.5 px-3 py-1.5 rounded-xl {{ request()->is('wishlist') ? 'text-orange-600' : 'text-gray-500' }} transition">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z"/></svg>
            <livewire:wishlist-counter />
            <span class="text-[10px] font-medium">{{ __('Wishlist') }}</span>
        </a>
        <a href="{{ auth()->check() ? '/my-account' : '/login' }}" class="flex flex-col items-center gap-0.5 px-3 py-1.5 rounded-xl {{ request()->is('my-account') ? 'text-orange-600' : 'text-gray-500' }} transition">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/></svg>
            <span class="text-[10px] font-medium">{{ auth()->check() ? __('Account') : __('Login') }}</span>
        </a>
    </nav>
