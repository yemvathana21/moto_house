...
<body class="bg-gray-50 text-gray-900 antialiased font-sans overflow-x-hidden pb-16 md:pb-0">
    <header class="bg-white border-b border-gray-100 sticky top-0 z-50 shadow-sm" x-data="{ mobileOpen: false }">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex items-center justify-between h-16">
                ...
                <div class="hidden md:block flex-1 max-w-xs mx-4" x-data="{ q: '', results: [], open: false, timer: null }"
                    ...
                    <div class="relative">
                        <input type="text" x-model="q"
                               ...
                               @keydown.enter="if (results.length) window.location = '/shop/' + results[0].slug"
                               placeholder="{{ __('Search...') }}"
                               class="w-full pl-10 pr-4 py-2 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-transparent focus:bg-white transition">
                        <svg class="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg>
                        <button x-show="q" @click="q = ''; results = []; open = false" type="button" class="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
                        </button>
                    </div>
                    <div x-show="open" class="absolute mt-2 w-full bg-white rounded-xl shadow-lg border border-gray-100 py-2 z-50 max-h-80 overflow-y-auto" x-cloak>
                        <template x-for="p in results" :key="p.id">
                            <a :href="'/shop/' + p.slug" class="flex items-center gap-3 px-4 py-2.5 hover:bg-orange-50 transition">
    ...
    <nav class="md:hidden fixed bottom-0 inset-x-0 bg-white border-t border-gray-200 z-50 flex items-center justify-between px-1 py-1" style="padding-bottom: calc(0.25rem + env(safe-area-inset-bottom, 0px))" x-data>
        <a href="/" class="flex flex-col items-center gap-0.5 px-1 py-1.5 rounded-xl min-w-0 flex-1 {{ request()->is('/') ? 'text-orange-600' : 'text-gray-500' }} transition">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6"/></svg>
            <span class="text-[9px] font-medium truncate w-full text-center">{{ __('Home') }}</span>
        </a>
        <a href="/shop" class="flex flex-col items-center gap-0.5 px-1 py-1.5 rounded-xl min-w-0 flex-1 {{ request()->is('shop') || request()->is('shop/*') ? 'text-orange-600' : 'text-gray-500' }} transition">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z"/></svg>
            <span class="text-[9px] font-medium truncate w-full text-center">{{ __('Shop') }}</span>
        </a>
        <a href="/cart" class="flex flex-col items-center gap-0.5 px-1 py-1.5 rounded-xl min-w-0 flex-1 {{ request()->is('cart') ? 'text-orange-600' : 'text-gray-500' }} transition relative">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 100 4 2 2 0 000-4z"/></svg>
            <livewire:cart-counter />
            <span class="text-[9px] font-medium truncate w-full text-center">{{ __('Cart') }}</span>
        </a>
        <a href="/wishlist" class="flex flex-col items-center gap-0.5 px-1 py-1.5 rounded-xl min-w-0 flex-1 {{ request()->is('wishlist') ? 'text-orange-600' : 'text-gray-500' }} transition">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z"/></svg>
            <livewire:wishlist-counter />
            <span class="text-[9px] font-medium truncate w-full text-center">{{ __('Wishlist') }}</span>
        </a>
        <a href="{{ auth()->check() ? '/my-account' : '/login' }}" class="flex flex-col items-center gap-0.5 px-1 py-1.5 rounded-xl min-w-0 flex-1 {{ request()->is('my-account') ? 'text-orange-600' : 'text-gray-500' }} transition">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/></svg>
            <span class="text-[9px] font-medium truncate w-full text-center">{{ auth()->check() ? __('Account') : __('Login') }}</span>
        </a>
    </nav>
